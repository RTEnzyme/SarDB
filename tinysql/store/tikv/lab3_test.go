...
var _ = Suite(&testLab3Suite{})

func (s *testLab3Suite) SetUpSuite(c *C) {
...
func (s *testLab3Suite) TearDownSuite(c *C) {
	CommitMaxBackoff = 20000
	s.store.Close()
	s.OneByOneSuite.TearDownSuite(c)
}

func (s *testLab3Suite) checkValues(c *C, m map[string]string) {
	txn := s.begin(c)
	for k, v := range m {
...
func (s *testLab3Suite) mustGetLock(c *C, key []byte) *Lock {
...
func (s *testLab3Suite) mustUnLock(c *C, key []byte) {
	ver, err := s.store.CurrentVersion()
	c.Assert(err, IsNil)
	bo := NewBackoffer(context.Background(), getMaxBackoff)
	req := tikvrpc.NewRequest(tikvrpc.CmdGet, &kvrpcpb.GetRequest{
...
func (s *testLab3Suite) begin(c *C) *tikvTxn {
	txn, err := s.store.Begin()
	c.Assert(err, IsNil)
	return txn.(*tikvTxn)
}

func (s *testLab3Suite) beginWithStartTs(c *C, ts uint64) *tikvTxn {
	txn, err := s.store.BeginWithStartTS(ts)
	c.Assert(err, IsNil)
	return txn.(*tikvTxn)
}

func (s *testLab3Suite) TestInitKeysAndMutations(c *C) {
	txn := s.begin(c)
...
func (s *testLab3Suite) preparePrewritedTxn(c *C, pk string, kvs map[string]string) (*tikvTxn, *twoPhaseCommitter) {
	txn := s.begin(c)
	hasPk := false
...
func (s *testLab3Suite) TestCommitSingleBatch(c *C) {
	k1 := []byte("a1")
	k2 := []byte("b1")
	k3 := []byte("b2")
...
func (s *testLab3Suite) TestRollbackSingleBatch(c *C) {
	k1 := []byte("a1")
	k2 := []byte("b1")
	k3 := []byte("b2")
...
func (s *testLab3Suite) TestRollback(c *C) {
	var err error
	k1 := []byte("k1")
	k2 := []byte("k2")
...
func (s *testLab3Suite) TestResolveLock(c *C) {
	k1 := []byte("a")
	k2 := []byte("b")
	k3 := []byte("c")
...
func (s *testLab3Suite) TestFailAfterPrimary(c *C) {
...
func (s *testLab3Suite) TestGetResolveLockCommit(c *C) {
	txn, committer := s.preparePrewritedTxn(c, "a", map[string]string{
		"a": "a",
		"b": "b",
	})
