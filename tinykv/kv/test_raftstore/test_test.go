...
func partitioner(t *testing.T, cluster *Cluster, ch chan bool, done *int32, unreliable bool, electionTimeout time.Duration) {
	...
	for atomic.LoadInt32(done) == 0 {
		...
		log.Info(fmt.Sprintf("partition: %v, %v", pa[0], pa[1]))
...
func GenericTest(t *testing.T, part string, nclients int, unreliable bool, crash bool, partitions bool, maxraftlog int, confchange bool, split bool) {
	...
	for i := 0; i < 3; i++ {
		...
		go SpawnClientsAndWait(t, ch_clients, nclients, func(cli int, t *testing.T) {
			...
			for atomic.LoadInt32(&done_clients) == 0 || (split && atomic.LoadUint64(&writeCnt) < cfg.RegionMaxSize) {
				if (rand.Int() % 1000) < 500 {
					...
					// log.Info(fmt.Sprintf("%d: client new put %v,%v\n", cli, key, value))
		...
		if crash {
			log.Warn(fmt.Sprintf("shutdown servers\n"))
			...
			time.Sleep(electionTimeout)
			log.Warn(fmt.Sprintf("restart servers\n"))
...
func TestOnePartition2Blab1P1b(t *testing.T) {
	cfg := config.NewTestConfig()
...
func TestSnapshotRecover2BLab1(t *testing.T) {
	// Test: restarts, snapshots, one client (2B) ...
	GenericTest(t, "2B", 1, false, true, false, 100, false, false)
}

func TestSnapshotRecoverManyClients2BLab1P3b(t *testing.T) {
	// Test: restarts, snapshots, many clients (2B) ...
	GenericTest(t, "2B", 20, false, true, false, 100, false, false)
}

func TestSnapshotUnreliable2BLab1(t *testing.T) {
	// Test: unreliable net, snapshots, many clients (2B) ...
	GenericTest(t, "2B", 5, true, false, false, 100, false, false)
}

func TestSnapshotUnreliableRecover2BLab1(t *testing.T) {
	// Test: unreliable net, restarts, snapshots, many clients (2B) ...
	GenericTest(t, "2B", 5, true, true, false, 100, false, false)
}

func TestSnapshotUnreliableRecoverConcurrentPartition2BLab1P3b(t *testing.T) {
	// Test: unreliable net, restarts, partitions, snapshots, many clients (2B) ...
	GenericTest(t, "2B", 5, true, true, true, 100, false, false)
}

func TestTransferLeader3BLab1P4a(t *testing.T) {
	cfg := config.NewTestConfig()
	cluster := NewTestCluster(5, cfg)
	cluster.Start()
	defer cluster.Shutdown()
...
func TestOneSplit3BLab1P4a(t *testing.T) {
	cfg := config.NewTestConfig()
...
func TestSplitConfChangeSnapshotUnreliableRecoverConcurrentPartition3BLab1P4b(t *testing.T) {
	// Test: unreliable net, restarts, partitions, snapshots, conf change, many clients (3B) ...
